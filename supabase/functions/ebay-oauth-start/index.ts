...
function addParam(urlStr, key, value) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const expires_in = tokenJson.expires_in ?? 0;
    const expires_at = new Date(Date.now() + Math.max(0, expires_in - 60) * 1000).toISOString();
    // Store tokens
    const supabase1 = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
    const { error: dbError } = await supabase1.from("oauth_tokens").upsert({
      provider: "ebay",
      access_token,
